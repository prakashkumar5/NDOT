...
$(document).ready(function() {
  ...
  var identity=$.urlParam('name');
  alert(identity);
  trip_channel_name=$.urlParam('id');
    if (!identity) { return; }
  ...
  $('#create-new-channel').on('click', function() {
    ...
    .then(page => {
      ...
      subscribedChannels.forEach(function(channel) {
        alert(channel.friendlyName);
        if(channel.friendlyName == "test")
        {
          exist=1;
          exist_channel=channel;
        }
      ...
      alert(exist);
        ...
        {
          alert(exist_channel);
          setActiveChannel(exist_channel);
        }
      });
...
function fn()
{
  ...
  var exist=0;
    ...
    .then(page => {
      ...
      subscribedChannels.forEach(function(channel) {
        alert(channel.friendlyName);
        if(channel.friendlyName == trip_channel_name)
        {
          exist=1;
          exist_channel=channel;
        }
      ...
      alert(exist);
        if(exist == 0)
        {
              client.createChannel({
             // attributes: attributes,
              friendlyName: trip_channel_name,
              //isPrivate: isPrivate,
              //uniqueName: uniqueName
            }).then(function joinChannel(channel) {
              $('#create-channel').hide();
